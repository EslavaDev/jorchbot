...
- [ ] **1A.1** Add `ClaudeRunnerSpawnError` to `src/errors/index.ts`
- [ ] **1A.2** Add `ClaudeRunnerParseError` to `src/errors/index.ts`
- [ ] **1A.3** Add `ClaudeRunnerTimeoutError` to `src/errors/index.ts`
- [ ] **1A.4** Add `ClaudeRunnerProcessError` to `src/errors/index.ts`
- [ ] **1A.5** Update `KapsoSchema` in `src/config/jorchbot-config.ts`: add `phoneNumberId: z.string().default("")`
- [ ] **1A.6** Update `KapsoSchema` in `src/config/jorchbot-config.ts`: add `webhookVerifyToken: z.string().default("")`
- [ ] **1A.7** Update existing config test if any, to cover new fields
- [ ] **1A.8** Verify that `pnpm check` passes
...
- [ ] **1B.1** Create `src/sessions/jorchbot/context-tracker.ts` with `calculateContextUsage()` and `formatContextUsage()`
- [ ] **1B.2** Create `src/sessions/jorchbot/context-tracker.test.ts`
...
- [ ] **1B.3** Verify tests pass: `pnpm test:fast -- src/sessions/jorchbot/context-tracker.test.ts`
...
- [ ] **1C.1** Create `extensions/kapso/src/types.ts` with:
...
- [ ] **1C.2** Create `extensions/kapso/src/client.ts` with `KapsoClient`:
...
- [ ] **1C.3** Create `extensions/kapso/src/client.test.ts`:
...
- [ ] **1C.4** Create `extensions/kapso/src/webhook.ts` with `createWebhookHandlers()`:
  - `verify()` — GET handler for Kapso challenge
  - `receive()` — POST handler for incoming messages
  - Parse text messages and button replies
- [ ] **1C.5** Create `extensions/kapso/src/webhook.test.ts`:
...
- [ ] **1C.6** Verify all tests: `pnpm test:fast -- extensions/kapso/`
...
- [ ] **1D.1** Create `extensions/kapso/src/runtime.ts` with `setKapsoRuntime()` and `getKapsoRuntime()`
- [ ] **1D.2** Create `extensions/kapso/src/channel.ts` with `kapsoPlugin: ChannelPlugin<ResolvedKapsoAccount>`:
...
- [ ] **1D.3** Create `extensions/kapso/src/channel.test.ts`:
...
- [ ] **1D.4** Activate `extensions/kapso/index.ts` — uncomment plugin definition, import channel + runtime
- [ ] **1D.5** Verify `openclaw.plugin.json` has the correct channel id
- [ ] **1D.6** Verify tests pass: `pnpm test:fast -- extensions/kapso/`
- [ ] **1D.7** Verify that `pnpm check` passes (no type errors in the plugin)
...
- [ ] **1E.1** Replace placeholder in `src/sessions/jorchbot/claude-runner.ts` with full implementation:
...
- [ ] **1E.2** Create `src/sessions/jorchbot/claude-runner.test.ts`:
...
- [ ] **1E.3** Verify tests pass: `pnpm test:fast -- src/sessions/jorchbot/claude-runner.test.ts`
- [ ] **1E.4** Verify that `pnpm check` passes
...
- [ ] **1F.1** Create `src/sessions/jorchbot/approval-manager.ts` with `ApprovalManager`:
...
- [ ] **1F.2** Create `src/sessions/jorchbot/approval-manager.test.ts`:
...
- [ ] **1F.3** Verify tests: `pnpm test:fast -- src/sessions/jorchbot/approval-manager.test.ts`
...
- [ ] **1G.1** Create `src/commands/router.ts` with `CommandRouter`:
...
- [ ] **1G.2** Create `src/commands/router.test.ts`:
...
- [ ] **1G.3** Verify tests: `pnpm test:fast -- src/commands/router.test.ts`
- [ ] **1G.4** Update `src/gateway/jorchbot-start.ts`:
...
- [ ] **1G.5** Register webhook routes on the gateway:
...
- [ ] **1G.6** Verify that `pnpm check` passes
...
- [ ] **1H.1** Create or update the `jorchbot setup` CLI command:
...
- [ ] **1H.2** Document manual setup in README or docs:
...
- [ ] **1H.3** Final end-to-end verification:
  - [ ] `pnpm check` passes (format + types + lint)
  - [ ] `pnpm test:fast` passes (all tests)
  - [ ] No `any` in new code
  - [ ] All imports use `.js` suffix
  - [ ] All type-only imports use `import type`
  - [ ] New files < 700 LOC
  - [ ] Errors chain `{ cause: err }` correctly
